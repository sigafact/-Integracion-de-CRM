...
const getBodyButton = (msg: any): string => {
  try {
    ...
    if (msg?.messageType === "interactiveMessage" || msg?.message?.interactiveMessage) {
      ...
      if (bodyTextWithPix) {
        bodyMessage += `[PIX]`;
        console.log("Se detectó un mensaje PIX, agregue [PIX] al cuerpo del mensaje.");
      } else {
        console.log("No se encontró ningún mensaje PIX.");
      }
...
const getBodyPIX = (msg: any): string => {
  try {
    ...
    console.error("Mensaje de error al procesar el mensaje:", error);
  }
...
const downloadMedia = async (msg: proto.IWebMessageInfo, isImported: Date = null, wbot: Session, ticket: Ticket) => {
  ...
  if (wbot && message?.fileLength && +message.fileLength > fileLimit * 1024 * 1024) {
    ...
    sendMsg.message.extendedTextMessage.text = "\u200e*Mensagem do sistema*:\nArchivo recibido fuera del límite de tamaño del sistema, en caso de ser necesario se puede obtener en la aplicación WhatsApp.";
  ...
  try {
    ...
    if (isImported) {
      console.log(
        "No se pudo descargar un mensaje importado, es probable que el mensaje ya no esté disponible"
      );
    } else {
...
export const verifyMediaMessage = async (
  ...
  try {
    ...
    if (!media && ticket.imported) {
      const body =
        "*System:* \nError en la descarga de medios, verifique el dispositivo";
      const messageData = {
        //mensagem de texto
        wid: msg.key.id,
        ticketId: ticket.id,
    ...
    console.log(error);
    logger.warn("Error al descargar el medio: ", JSON.stringify(msg));
  }
};
  ...
  const botText = async () => {
    ...
    if (choosenQueue || (queues.length === 1 && chatbot)) {
      ...
      if (enableQueuePosition && !choosenQueue.chatbots.length) {
        ...
        const qtd = count.count === 0 ? 1 : count.count;
        const msgFila = `${settings.sendQueuePositionMessage} *${qtd}*`;
        // const msgFila = `*Asistente virtual:*\n{{ms}} *{{name}}*, sSu posición en la cola de servicio es: *${qtd}*`;
        const bodyFila = formatBody(`${msgFila}`, ticket);
        const debouncedSentMessagePosicao = debounce(
          async () => {
            await wbot.sendMessage(
    ...
    if (choosenQueue || (queues.length === 1 && chatbot)) {
      ...
      if (choosenQueue.chatbots.length > 0 && !ticket.isGroup) {
        ...
        const debouncedSentMessage = debounce(
          async () => {
            try {
              console.log("log... opciones de cola de envío");
              ...
              if (!whatsapp || !whatsapp.number) {
                console.error('No se encontró el número de WhatsApp para el ticket:', ticket.whatsappId);
                throw new Error('No se encontró el número de WhatsApp');
              }
              ...
              buttons.push({
                name: 'quick_reply',
                buttonParamsJson: JSON.stringify({
                  display_text: "Volver al menú de inicio",
                  id: "#"
                })
              });
  ...
  try {
    ...
    if (
      ...
      const sentMessage = await wbot.sendMessage(
        `${contact.number}@c.us`,
        {
          text: `\u200e*Asistente virtual*:\nLamentablemente no podemos escuchar ni enviar audios a través de este canal de servicio, por favor envíe un mensaje de *texto*.`
        },
        {
          quoted: {
            key: msg.key,
