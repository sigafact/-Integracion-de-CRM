...
const FlowBuilderSingleBlockModal = ({
  ...
  const [labels, setLabels] = useState({
    title: "Añadir contenido al stream",
    btn: "Para agregar",
  });
  ...
  const handleElements = (newNameFiles) => {
    ...
    for (let i = 0; i < numberMessages; i++) {
      ...
      if (!value) {
        toast.error("Campos de mensajes vacíos!");
        setLoading(false);
        throw "";
      }
  ...
  const handleSaveNode = async () => {
    if (open === "edit") {
      ...
      setTimeout(async () => {
        ...
        if (verify) {
          setLoading(false);
          return toast.error("Eliminar tarjetas vacías(Imagem, Audio e Video)");
        }
        await api
...
>
  <p>Variables</p>
  <p><strong>{"{{firstName}}"}</strong>: Devuelve el primer nombre del contacto relacionado con el ticket. Si no hay contacto, devuelve una cadena vacía.</p>
  <p><strong>{"{{name}}"}</strong>: El nombre completo del contacto (o una cadena vacía si no hay contacto).</p>
  <p><strong>{"{{userName}}"}</strong>: El nombre del usuario asociado con el ticket. Si no hay usuario, devuelve una cadena vacía.</p>
  <p><strong>{"{{ms}}"}</strong>: Devuelve un saludo basado en la hora actual (por ejemplo, "Buenos Días", "Buenas Tardes").</p>
  <p><strong>{"{{protocol}}"}</strong>: Combinación de control() y el ID del ticket, creando un protocolo único.</p>
  <p><strong>{"{{date}}"}</strong>: Devuelve la fecha actual en el formato dd-mm-yyyy.</p>
  <p><strong>{"{{hour}}"}</strong>: Devuelve la hora actual en el formato hh:mm:ss.</p>
  <p><strong>{"{{ticket_id}}"}</strong>: El ID del ticket, si existe. De lo contrario, devuelve una cadena vacía.</p>
  <p><strong>{"{{queue}}"}</strong>: Devuelve el nombre de la cola a la que está asociado el ticket (si existe).</p>
  <p><strong>{"{{connection}}"}</strong>: Devuelve el nombre de la conexión de WhatsApp asociada con el ticket.</p>
</Typography>
